...
var maxLatency = flag.Duration("max-latency", 5*time.Minute, "Maximum time a query is expected to take (used as a histogram cutoff)")
var minLatency = flag.Duration("min-latency", 500*time.Millisecond, "Minimum time a query is expected to take (used as a histogram cutoff)")
...
func loopQueries(ctx context.Context, wg *sync.WaitGroup, errChan chan<- error, id int, db *sql.DB, queries []string, histograms []*sharedHistogram) {
	...
	for i := uint(0); i < *loops || *loops == 0; i++ {
		for j, query := range queries {
			...
			histograms[j].Lock()
			histograms[j].RecordValue(elapsed.Nanoseconds())
			histograms[j].numOps++
			histograms[j].Unlock()
			if *verbose {
...
func main() {
	...
	for i := range histograms {
		histograms[i] = &sharedHistogram{Histogram: hdrhistogram.New(minLatency.Nanoseconds(), maxLatency.Nanoseconds(), 1 /* sigfig */)}
	}
