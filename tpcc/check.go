...
((SELECT w_ytd FROM warehouse ORDER BY w_id)
 EXCEPT ALL
(SELECT SUM(d_ytd) FROM district GROUP BY d_w_id ORDER BY d_w_id))
UNION ALL
((SELECT SUM(d_ytd) FROM district GROUP BY d_w_id ORDER BY d_w_id)
 EXCEPT ALL
(SELECT w_ytd FROM warehouse ORDER BY w_id))
...
func checkConsistency(db *sql.DB) bool {
	// TODO(arjun): each query should only require a single connection at a time, this should not be required.
