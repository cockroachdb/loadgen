...
var concurrency = flag.Int("concurrency", 8,
...
func (yw *ycsbWorker) insertRow(key uint64) error {
	...
	insertString := fmt.Sprintf(insertStmt, buffer.String())
...
func setupDatabase(dbURL string) (*sql.DB, error) {
	...
	if *drop {
		...
		err = crdb.ExecuteTx(db, func(tx *sql.Tx) error {
			if _, inErr := db.Exec(dropStmt); inErr != nil {
				return inErr
			}
			return nil
		})
...
func main() {
	...
	for i := 0; ; {
		...
		case <-tick:
			...
			if i%20 == 0 {
				fmt.Printf("               elapsed       ops/sec   emptyReads nonEmptyReads      writes        scans   readErrors   scanErrors  writeErrors\n")
			}
			statsString := fmt.Sprintf(" %12d %12d %12d %12d %12d %12d %12d",
			...
			printStr := fmt.Sprintf("%s\t%6s %9.1f/sec", *benchmarkName,
