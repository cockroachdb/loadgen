...
// SQL statements
const (
	insertStmt = `INSERT INTO usertable VALUES (%s)`
	selectStmt = `SELECT * FROM usertable WHERE ycsb_key=%d`
	createStmt = `
	...
	dropStmt    = `DROP TABLE IF EXISTS usertable`
	numFields   = 10
...
func (yw *ycsbWorker) insertRow(key uint64) error {
	fields := make([]string, numFields+1)
	fields[0] = fmt.Sprintf("%d", key)
	for i := 1; i < len(fields); i++ {
		fields[i] = yw.randString(fieldLength)
	}
	values := strings.Join(fields, ", ")
	// TODO(arjun): Consider using a placeholder here.
	insertString := fmt.Sprintf(insertStmt, values)
...
func (yw *ycsbWorker) readRow() error {
	key := yw.nextReadKey()
	readString := fmt.Sprintf(selectStmt, key)
...
func main() {
	...
	for i := 0; ; {
		...
		case <-tick:
			...
			if i%20 == 0 {
				fmt.Printf("_______________elapsed_______ops/sec___emptyReads_nonEmptyReads______writes________scans____readErrors___scanErrors__writeErrors\n")
			}
			fmt.Printf("%6s %9.1f/sec %12d %12d %12d %12d %12d %12d %12d",
