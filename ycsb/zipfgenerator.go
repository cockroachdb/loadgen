...
func NewZipfGenerator(iMin, iMax uint64, theta float64, verbose bool) (*ZipfGenerator, error) {
	...
	if theta == 1.0 {
		return nil, errors.Errorf("theta cannot be exactly 1")
	}
	...
	z.zeta, err = computeZetaFromScratch(iMax+1-iMin, theta)
	if err != nil {
		return nil, errors.Errorf("Could not compute zeta(2,%d): %s", iMax, err)
	}
	z.alpha = 1.0 / (1.0 - theta)
	z.zipfGenMu.eta = (1 - math.Pow(2.0/float64(z.zipfGenMu.iMax+1-z.iMin), 1.0-z.theta)) / (1.0 - z.zeta2/z.zeta)
	return &z, nil
}
...
func computeZetaFromScratch(n uint64, theta float64) (float64, error) {
	//fmt.Println("Debug: computeZetaFromScratch")
	var err error
	var zeta float64
	if zeta, err = computeZetaIncrementally(0, n, theta, 0.0); err != nil {
...
func (z *ZipfGenerator) IncrementIMax() (uint64, error) {
	...
	var err error
	z.zeta, err = computeZetaIncrementally(z.zipfGenMu.iMax, z.zipfGenMu.iMax+1, z.theta, z.zeta)
	z.zipfGenMu.eta = (1 - math.Pow(2.0/float64(z.zipfGenMu.iMax+1-z.iMin), 1.0-z.theta)) / (1.0 - z.zeta2/z.zeta)
	if err != nil {
		return 0, errors.Errorf("Could not incrementally compute zeta: %s", err)
	}
	z.zipfGenMu.iMax++
	return z.zipfGenMu.iMax - 1, nil
}

func (z *ZipfGenerator) int() int {
	return int(z.Uint64())
}
